...
const AddParkingPage: React.FC = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-6">Add New Parking Facility</h1>
      ...
      <div className="max-w-2xl mx-auto bg-white rounded-lg shadow-md p-6">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div>
            ...
            <input
              ...
              {...register('code', {
                ...
                pattern: {
                  value: /^[A-Z0-9]+$/,
                  message: 'Parking code must contain only uppercase letters and numbers',
                },
              })}
          ...
          <div>
            <label htmlFor="charge_per_hour" className="block text-sm font-medium text-gray-700">
              Charge per Hour ($)
            </label>
            <input
              type="number"
              ...
              {...register('charge_per_hour', {
                required: 'Charge per hour is required',
                min: {
                  value: 0,
                  message: 'Charge per hour must be 0 or greater',
                },
              })}
              className={`mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 ${
                errors.charge_per_hour ? 'border-red-500' : ''
              }`}
            />
            {errors.charge_per_hour && (
              <p className="mt-1 text-sm text-red-600">{errors.charge_per_hour.message}</p>
            )}
          </div>
